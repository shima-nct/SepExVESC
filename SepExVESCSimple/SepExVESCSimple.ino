...
// --- 界磁電流 ---
constexpr float FIELD_CURRENT_MA = 1000.0f; // 界磁電流[mA]
...
void vescControlTask(void *pvParameters) {
  ...
  while (1) {
    ...
    int32_t scaled_field_current =(int32_t)(FIELD_CURRENT_MA * 1000);

    // CAN メッセージ作成
    twai_message_t armature_msg;
    memset(&armature_msg, 0, sizeof(armature_msg)); // メッセージ構造体をゼロクリア

    // 界磁電流を設定するためのCANメッセージを作成します。
    twai_message_t field_msg;
    memset(&field_msg, 0, sizeof(field_msg)); // メッセージ構造体をゼロクリア
    ...
    // 界磁電流を設定するためのCANメッセージのデータをビッグエンディアン形式のバイト配列に変換します。
